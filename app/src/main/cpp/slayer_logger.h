...
namespace hackback {
    ...
    public:
        ...
        [[maybe_unused]] void back_echo(const std::string format, Args... variables) {
            ...
            __android_log_write(static_cast<int>(m_current_priority),
                                gs_backend_tag.data(),
                                reinterpret_cast<const char*>(message_buffer.data()));
        }

        template<typename... Args>
        [[maybe_unused]] void back_echo(android_LogPriority prior, std::string_view format, Args... variables) {
            change_level(prior);
            back_echo(format, variables...);
        }

        void back_echo(const std::string format) {

            auto message_buffer = fmt::memory_buffer();
            message_buffer.reserve(format.size() * 1.30);
            format_level(message_buffer);
            fmt::format_to(std::back_inserter(message_buffer), format);
            __android_log_write(static_cast<int>(m_current_priority),
                                gs_backend_tag.data(),
                                reinterpret_cast<const char*>(message_buffer.data()));
        }
    ...
    private:
        ...
        void format_level(fmt::memory_buffer& out_buffer) {
            ...
            case ANDROID_LOG_UNKNOWN:   assert("Unknown priority level specified" == nullptr);
            case ANDROID_LOG_DEFAULT:   trap_not_impl(); break;
            case ANDROID_LOG_VERBOSE:   trap_not_impl(); break;
            case ANDROID_LOG_DEBUG:
                fmt::format_to(std::back_inserter(out_buffer),
                               "Slayer [Debug]: ");
                break;
